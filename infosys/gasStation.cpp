...
public:
    int canCompleteCircuit(vector<int>& gas, vector<int>& cost) {
        ...
        //for calculating how much gas we need for traveling all the gas station
        int totalGas = 0;
        
        //for claculating how much fuel we need for travelling ith index top i+1th index
        int currGas = 0;
        
        //note starting gas station
        int start = 0;
        
        //start calculating from 0
        for(int i=0;i<n;i++){
            
            //calculate how much  gas we need  for traveling all the gas station
            totalGas += gas[i] - cost[i];
            
             //claculate how much we need gas for travelling ith index top i+1th index
            currGas += gas[i] - cost[i];
            
            //if we have lesser gas 
            if(currGas < 0){
                currGas = 0;
                start = i+1;
            }
        ...
        //if we have less gas for travelling all the station return -1
        if(totalGas < 0)
            return -1;
        
        //else return stating point
        return start;
    }
};
